...
WHERE m.nombre_materiales = 'DataScience'
ORDER BY a.Nombre_alumno;
...
WHERE a.id_alumno = 1
ORDER BY m.nombre_materiales, p.nombre_proyecto;
...
FROM Proyectos p
INNER JOIN Materias m ON p.materia_id = m.id_materia
GROUP BY p.nombre_proyecto, m.nombre_materiales
...
GROUP BY m.id_materia, m.nombre_materiales;
...
GROUP BY e.id_edicion, m.nombre_materiales, e.Fecha, mod.horario, mod.tipo, cl1.Nombre_claustro, cl2.Nombre_claustro
...
INNER JOIN Campus c ON e.Campus_id = c.id_campus
INNER JOIN modalidades mod ON e.Modalidad_id = mod.id_modalidad
ORDER BY a.Nombre_alumno, m.nombre_materiales;
